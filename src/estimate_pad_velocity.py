...
def tf_callback(tf2):
    ...
    global p_v_pub, latest_common_time
    try:
        t = tf_.getLatestCommonTime('/world', targeted_tf)
        if latest_common_time < t:
            latest_common_time = t
            position, quaternion = tf_.lookupTransform('/world', targeted_tf, t)
            ps = PoseStamped()
